...
{
    public class GetLastFourOrdersQueryHandler(AppDbContext context, IMapper mapper)
    {
        public async Task<List<GetOrdersQueryResult>> Handle()
        {
            var orders = await context.Orders.Include(o => o.User).AsNoTracking().Take(4).ToListAsync();
